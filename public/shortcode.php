...
if ( ! function_exists( 'broke_shortcode' ) ) :

  function broke_shortcode( $atts, $content = null, $tag, $class = null ) {
    ...
    if ( empty( $atts['id'] ) ) {
      ...
      if ( empty( $atts['conditions'] ) && $tag !== NULL ) {
        return sprintf( '<p>%s</p>', __( 'Please select a template to render.', 'broke-templates' ) );
      ...
      $autoload = false;
        ...
        $source = broke_allowed_data_sources( get_post_meta( get_the_ID(), '_broke_data_source', true ) );

        $code = broke_kses( get_post_meta( get_the_ID(), '_broke_template_code', true ) );

        $placement = get_post_meta( get_the_ID(), '_broke_attribute_placement', true );

				$matched_conditions = get_post_meta( get_the_ID(), '_broke_condition', false );
        ...
        if ( $source == 'single' ) {
          ...
          $data_query = (array) get_post_meta( get_the_ID(), '_broke_data_query', true );
        ...
        $code = Timber::compile_string( $code, $context );
				...
				do_action( 'broke_rendering', array(
